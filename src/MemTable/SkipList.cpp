...
void SkipList::put(uint64_t k, const std::string &v) {//跳转表词条插入算法
  ...
  if (empty())
    skip_list_.push_back(new QuadList); //插入首个Entry

  auto qlist = skip_list_.begin();  //从顶层四联表的
  ...
  if (skipSearch(qlist, p, k))  //查找适当的插入位置（不大于关键码k的最后一个节点p）
  {
    ...
    while (rand() & 1) {                                  //经投掷硬币，若确定新塔需要再长高一层，则
      ...
      if (!(*qlist)->valid(p)) {                          //若该前驱是header
        if (qlist == skip_list_.begin())                  //且当前已是最顶层，则意味着必须
        {
          QuadList *new_quadlist = new QuadList;
          skip_list_.push_front(new_quadlist);//首先创建新的一层，然后
        }
...
std::string *SkipList::get(uint64_t k) {
  ...
  auto qlist = skip_list_.begin();        //从顶层
  QListNodePosi p = (*qlist)->first();//首节点出发
...
bool SkipList::remove(uint64_t k, uint32_t &length) {
  if (skip_list_.empty()) return false;   //根据cppreference，当链表为空时，引用begin，end之类的东西是不行的
  auto qlist = skip_list_.begin();        //从顶层
  ...
  if (skipSearch(qlist, p, k)) {      //找到该节点，删除这一整座塔
    ...
    while (!skip_list_.empty() && skip_list_.front()->empty()) {//删除空了的顶层，同理，当你使用front时，要先检查链表是不是空了
